...
transform:
  ...
  gemini: section
  codex: section
skills: [general-style, oop-principles, spring-boot-conventions, java-style, kotlin-style, layered-architecture, java-testing, kotlin-testing, implementation-doc, implementation-checkpoint]
---
...
- 설계 문서의 범위를 벗어나는 작업은 하지 않는다. 구현에 필요한 부수 변경(의존성 추가, 설정, import 정리)은 범위 안으로 본다. 설계에 없는 기능 추가나 무관한 코드 리팩터링은 하지 않고, 필요하다고 판단되면 멈추고 사용자에게 확인한다.
